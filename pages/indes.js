...
export default function Home() {
  ...
  const simulateGift = (gift, who) => {
    const ts = new Date().toLocaleTimeString()
    setLogs(prev => [...prev, { ts, text: `${who} wysłał ${gift}` }])
  }
    ...
    <div>
      ...
      <div className="card" style={{ marginTop: 12 }}>
        ...
        <div style={{ marginTop: 10, display: 'flex', gap: 8 }}>
          <button className="smallBtn" onClick={() => setLogs([])}>
            Wyczyść
          </button>
        </div>
      </div>
      ...
      <div style={{ marginTop: 12 }} className="card">
        ...
        <ul style={{ color: 'var(--muted)' }}>
          ...
          <li>
            ServerTap musi udostępniać endpointy: <code>GET /v1/server</code> oraz <code>POST /v1/execute</code> (z
            headerem <code>x-api-key</code>).
          </li>
          <li>
            Jeśli serwer jest za NAT/em lub nie ma CORS — użyj tunelu (ngrok) lub prostego proxy.
          </li>
        </ul>
      </div>
